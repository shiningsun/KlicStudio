...
🎯 **Khởi động một cú nhấp**: Không cần cấu hình môi trường phức tạp, tự động cài đặt phụ thuộc, sẵn sàng sử dụng ngay lập tức, phiên bản desktop mới, sử dụng thuận tiện hơn!
...
🎙️ **Nhân bản giọng nói**: Cung cấp giọng nói được chọn lọc từ CosyVoice hoặc nhân bản giọng nói tùy chỉnh
...
## 🚀 Bắt đầu nhanh
### Các bước cơ bản
Đầu tiên tải xuống tệp thực thi tương ứng với hệ điều hành của bạn từ [Release](https://github.com/krillinai/KrillinAI/releases), theo hướng dẫn dưới đây để chọn phiên bản desktop hay không desktop, sau đó đặt vào một thư mục trống, tải phần mềm vào một thư mục trống, vì sau khi chạy sẽ tạo ra một số thư mục, đặt vào thư mục trống sẽ dễ quản lý hơn.  
...
【Nếu là phiên bản không desktop, tức là tệp release không có đuôi desktop, xem tại đây】  
...
1. Mở terminal trong thư mục chứa tệp thực thi (giả sử tên tệp là KrillinAI_1.0.0_desktop_macOS_arm64)
...
1. Mở terminal trong thư mục chứa tệp thực thi (giả sử tên tệp là KrillinAI_1.0.0_macOS_arm64)
2. Thực hiện lần lượt các lệnh sau:
   ```
...
Cách cấu hình sử dụng mô hình nhận diện ngôn ngữ cục bộ (hiện tại không hỗ trợ macOS) (cân nhắc chi phí, tốc độ và chất lượng)
* Điền `transcription_provider` là `fasterwhisper`, `llm_provider` là `openai`, như vậy trong ba mục cấu hình lớn `openai`、`local_model` chỉ cần điền `openai.apikey` và `local_model.faster_whisper` là có thể thực hiện dịch phụ đề, mô hình cục bộ sẽ tự động tải xuống. (`app.proxy` và `openai.base_url` tương tự)
...
* Nếu đã bật chức năng "lồng ghép giọng nói", đồng thời tải lên âm thanh cục bộ để nhân bản giọng nói, thì cũng cần sử dụng dịch vụ lưu trữ đám mây OSS của Alibaba Cloud, vì vậy cần điền mục `aliyun.oss`  
