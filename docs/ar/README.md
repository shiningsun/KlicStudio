<div align="center">
  ...
  # أداة ترجمة الفيديو والصوت بالذكاء الاصطناعي
...
Krillin AI هو حل شامل لتوطين وتعزيز الصوت والفيديو. هذه الأداة البسيطة والقوية تجمع بين ترجمة الفيديو، والتعليق الصوتي، واستنساخ الصوت، وتدعم إخراج التنسيقات الأفقية والعمودية، مما يضمن عرضًا مثاليًا على جميع المنصات الرئيسية (بيلبلي، شياوهونغشو، دوين، فيديوهات، كوايشو، يوتيوب، تيك توك، إلخ). من خلال سير العمل من البداية إلى النهاية، يمكن لـ Krillin AI تحويل المواد الأصلية إلى محتوى متعدد المنصات جاهز للاستخدام بنقرات قليلة فقط.

## الميزات والوظائف الرئيسية:
🎯 **تشغيل بنقرة واحدة**: لا حاجة لتكوين بيئة معقدة، يتم تثبيت الاعتماديات تلقائيًا، ويمكنك البدء على الفور، مع إصدار سطح مكتب جديد لسهولة الاستخدام!
...
🌍 **ترجمة احترافية**: استنادًا إلى LLM، تضمن الترجمة على مستوى الفقرات تماسك المعنى
...
| **FasterWhisper**  | ويندوز/لينكس     | `tiny`/`medium`/`large-v2` (يوصى بـ medium+) | محلي    | أسرع، بدون تكاليف سحابية |
...
## 🚀 البدء السريع
### الخطوات الأساسية
أولاً، قم بتنزيل [الإصدار](https://github.com/krillinai/KrillinAI/releases) الذي يتوافق مع نظام جهازك، ثم اختر بين الإصدار المكتبي أو غير المكتبي وفقًا للدليل أدناه، ثم ضع الملفات في مجلد فارغ، لأن البرنامج سيولد بعض الدلائل بعد التشغيل، وضعها في مجلد فارغ سيسهل إدارتها.  

【إذا كان الإصدار المكتبي، أي ملف الإصدار يحمل desktop، انظر هنا】  
...
1. انقر نقرًا مزدوجًا على الملف لبدء الاستخدام (يحتاج الإصدار المكتبي أيضًا إلى تكوين داخل البرنامج)

【إذا كان الإصدار غير المكتبي، أي ملف الإصدار لا يحمل desktop، انظر هنا】  
_الإصدار غير المكتبي هو الإصدار الأول، تكوينه معقد نسبيًا، ولكنه مستقر، ومناسب للنشر على الخوادم، حيث سيتم توفير واجهة المستخدم عبر الويب_
1. أنشئ مجلدًا باسم `config` داخل المجلد، ثم أنشئ ملفًا باسم `config.toml` داخل مجلد `config`، انسخ محتوى ملف `config-example.toml` الموجود في دليل `config` وأدخل معلومات التكوين الخاصة بك.
...
في الحالات التالية، تحتاج إلى تكوين علي بابا:
* إذا تم ملء `llm_provider` بـ `aliyun`، تحتاج إلى استخدام خدمة النموذج الكبير من علي بابا، لذا تحتاج إلى تكوين `aliyun.bailian`
* إذا تم ملء `transcription_provider` بـ `aliyun`، أو إذا تم تفعيل وظيفة "التعليق الصوتي" عند بدء المهمة، تحتاج إلى استخدام خدمة الصوت من علي بابا، لذا تحتاج إلى ملء `aliyun.speech`
* إذا تم تفعيل وظيفة "التعليق الصوتي" وتم رفع صوت محلي لاستنساخ الصوت، تحتاج أيضًا إلى استخدام خدمة تخزين سحابي OSS من علي بابا، لذا تحتاج إلى ملء `aliyun.oss`  
...
## معايير المساهمة
1. لا تقم بتقديم ملفات غير مفيدة، مثل .vscode و.idea، يرجى استخدام .gitignore بشكل فعال
...
2. تابع حساباتنا على وسائل التواصل الاجتماعي، [بيلبلي](https://space.bilibili.com/242124650)، حيث نشارك محتوى عالي الجودة في مجال تكنولوجيا الذكاء الاصطناعي يوميًا
